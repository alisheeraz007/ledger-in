...
const SaleList = (props) => {
      ...
      if (a.returnInvoice) {
        arr2.push({
          ...
          main_Heads: a.returnInvoice.main_heads
            ...
            : a.returnInvoice.main_Heads,
          SaleItem: a.returnInvoice.retrunInvoiceArr ? [...a.returnInvoice.retrunInvoiceArr] : [],
  ...
  function paginationFn(
    { pageSize, pageNumber },
    Entry = TodayFilter(
      entrySep(props.AccountReducer.All_entries).sort(
        (a, b) =>
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cellContent, row,rowIndex) => (
        <div>
          ...
          {row.invoiceNumber && row.SaleItem ? <>Sold To {row.SaleItem[0]?.customer.CustomerName} <b onClick={() => setModalId(entities[rowIndex])} style={{cursor:'pointer'}}>({row.invoiceNumber})</b> </>
            : <>
              Returned From {row.SaleItem ? row.SaleItem[0]?.customer.CustomerName : ''}
            </>}
        </div>
      ),
    },
    ...
    },
    {
      ...
      // },
      // formatter: (cellContent, row, rowindex) => {
      //   return <>{row.main_Heads ? row.main_Heads : row.main_heads}</>;
      // },
    },
    {
      ...
      formatter: (cellContent, row, rowIndex) => {
        return (
          <div onClick={()=>console.log(row)}>
            {row.netAmount ? row.netAmount : null}
          </div>
        );
      },
    },
