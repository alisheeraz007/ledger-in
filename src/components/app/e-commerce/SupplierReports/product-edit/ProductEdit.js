...
function ProductEdit(props) {
  ...
  useEffect(() => {
    ...
    let arr = props.ProductReducer.entities.filter(
      (obj) =>
        obj.bar_code.slice(3, 5) == makingLength_2(new Date().getMonth() + 1)
    );
    if (arr.length < 1) {
      Code(1);
    } else {
  ...
  const saveProduct = (values) => {
    ...
    if (rowId && proId) {
      ...
      if (!id) {
        ...
        props.AddSupProduct(values);
        backToProductsList();
      } else {
        props.EditProduct(values, id)
        props.EditSupProduct(values, id);
        backToProductsList();
      }
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    EditSupProduct: (id) =>
      dispatch(SupplierProductMiddileware.EditProduct(id)),
    AddSupProduct: (id) => dispatch(SupplierProductMiddileware.AddProduct(id)),
  };
}
